 ...
 * ```
 */
export function computeTargetSize(svg: string, pixelRatio: number, fallback: { width: number; height: number }) {
  ...
  if (!w) w = fallback.width;
  if (!h) h = fallback.height;
 ...
 * @internal
 */
function svgToDataURLCandidates(svg: string): string[] {
  const normalized = ensureXmlns(svg);
  const utf8 = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(normalized);
  let b64: string | null = null;
  try { b64 = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(normalized))); } catch { b64 = null; }
 ...
 * ```
 */
export async function svgToBitmap(svg: string, width: number, height: number): Promise<HTMLCanvasElement> {
  // Normalize boolean data attributes (data-fg / data-bg) to XML-friendly form
  svg = svg.replace(/(\sdata-(?:fg|bg))(\s|>|\/>)/gi, (_m, a: string, b: string) => `${a}="true"${b}`);
  // Prefer data URL (base64 first, then utf-8) to avoid blob loading issues across engines
  const toCanvas = async (src: string) => new Promise<HTMLCanvasElement>((resolve, reject) => {
    const img = new Image();
  ...
  for (const url of candidates) {
    try { return await toCanvas(url); } catch {}
  }
...
export class OverlaySvg extends BaseOp {
   ...
   */
  constructor(svgIcons: Record<string, string>, id: string = 'overlaySvg') { super(id); this.svgIcons = svgIcons; }
  async run(ctx: CanvasRenderingContext2D, W: number, H: number, params: Record<string, string>) {
    ...
    for (const overlayId of ids) {
      ...
      if (!raw) continue;
      let svg = raw;
      if (fg || bg) svg = applySvgFgBg(svg, fg, bg, { debug: !!params.logSvg });
      if (params.logSvg) {
        try { console.debug('[overlaySvg] svg after fg/bg', overlayId, svg); } catch {}
      }
      ...
      try {
        ...
        ctx.drawImage(bmp as any, ox, oy, targetW, targetH);
        if (params.debugRect) {
          try { ctx.save(); ctx.strokeStyle = '#0ff'; ctx.lineWidth = 1; ctx.strokeRect(Math.floor(ox)+0.5, Math.floor(oy)+0.5, Math.ceil(targetW)-1, Math.ceil(targetH)-1); ctx.restore(); } catch {}
        }
      } catch (e) {
        try {
          const ox = params.ox ? Number(params.ox) : (anchor.includes('right') ? W - targetW : anchor.includes('left') ? 0 : (W - targetW) / 2);
 ...
 * ```
 */
export function overlaySvg(svgIcons: Record<string, string>) {
