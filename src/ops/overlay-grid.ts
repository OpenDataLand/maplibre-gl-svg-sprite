...
export class OverlayGrid extends BaseOp {
   ...
   */
  constructor(svgIcons: Record<string, string>, id: string = 'overlayGrid') { super(id); this.svgIcons = svgIcons; }
   ...
   */
  measure(params: Record<string, string>): { width: number; height: number } {
    ...
    const cellBgIcon = (params.cellBgIcon || (params['cell-bg-icon'] as any) || params.badge || '') as string;
  ...
  async run(ctx: CanvasRenderingContext2D, W: number, H: number, params: Record<string, string>) {
    ...
    const cellBg = (params.cellBg || (params['cell-bg'] as any) || params.iconBg || params.background || '') as string;
    const cellPad = params.cellPad ? Number(params.cellPad) : (params.padding ? Number(params.padding) : 2);
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (!raw) {
        ...
        ctx.save(); ctx.strokeStyle = '#f00'; ctx.lineWidth = 1.5; ctx.strokeRect(x + 1, y + 1, Math.max(1, iw - 2), Math.max(1, ih - 2)); ctx.restore();
        // eslint-disable-next-line no-console
        console.warn('[overlayGrid] missing icon id:', id);
        continue;
      }
      let svg = raw;
      if (fg || bg) svg = applySvgFgBg(svg, fg, bg);
      try {
          ...
          if (cellBgIcon) {
            ...
            if (rawBg) {
              ...
              try {
                ...
                ctx.drawImage(bgBmp as any, bx + (bw - tw) / 2, by + (bh - th) / 2, tw, th);
              } catch (e) {
        ...
        ctx.drawImage(bmp as any, dx, dy, drawW, drawH);
      } catch (e) {
        const col = i % cols; const row = Math.floor(i / cols);
 ...
 * ```
 */
export function overlayGrid(svgIcons: Record<string, string>) {
  const op = new OverlayGrid(svgIcons);
  const fn: any = (ctx: CanvasRenderingContext2D, w: number, h: number, params: Record<string, string>) => op.run(ctx, w, h, params);
  fn.measure = (params: Record<string, string>) => op.measure(params);
  return fn as (typeof op.run & { measure?: (params: Record<string, string>) => { width: number; height: number } });
}
