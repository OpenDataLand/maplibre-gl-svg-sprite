...
function readAnimatedAttribute(el: SVGElement, name: string, fallback: string): string {
  const direct = (el as any)[name] ?? (el as any)[toCamel(name)];
  if (direct && typeof direct === 'object') {
    const animVal = 'animVal' in direct ? direct.animVal : null;
    if (animVal) {
      if (typeof animVal.valueAsString === 'string') return animVal.valueAsString;
      if (typeof animVal.value === 'number') return String(animVal.value);
    }
    const baseVal = 'baseVal' in direct ? direct.baseVal : null;
    if (baseVal && typeof baseVal.valueAsString === 'string') return baseVal.valueAsString;
  }
  return fallback;
}

function snapshotAnimatedSvg(source: SVGSVGElement): SVGSVGElement {
  ...
  const queue: Array<{ parent: SVGElement; child: SVGElement }> = Array.from(source.children)
    .map((child) => ({ parent: clone, child: child as SVGElement }));

  while (queue.length) {
    const { parent, child } = queue.shift()!;
    if (child.nodeType !== Node.ELEMENT_NODE) continue;
    if (ANIMATE_TAGS.includes(child.tagName)) continue;
    ...
    queue.push(...Array.from(child.children).map((grand) => ({ parent: newChild, child: grand as SVGElement })));
  }

  return clone;
}

export function createAnimatedSvgImage(
  map: MapLike,
  id: string,
  svgMarkup: string,
  ...
  const mapAny = map as MapLike;
  const mapCanvas = typeof mapAny.getCanvas === 'function' ? mapAny.getCanvas() : mapAny.canvas;
  if (!mapCanvas) throw new Error('Map canvas not found on MapLibre instance.');
  const container = mapCanvas.parentElement;
  if (!container) throw new Error('Map canvas container not found.');
  ...
  if (!svgEl) throw new Error('Animated SVG markup must contain a root <svg>.');

  container.appendChild(host);

  try {
    (svgEl as any).setCurrentTime?.(0);
    (svgEl as any).unpauseAnimations?.();
  } catch {}

  const animations = Array.from(svgEl.querySelectorAll<SVGElement>(ANIMATE_TAGS.join(',')));
  let running = true;
  ...
  if (!ctx) throw new Error('Could not acquire 2D context for animation buffer.');
  ...
  const styleImage: StyleImage = {
    ...
    render() {
      if (!running && remaining <= 0 && !pending) return false;
      ...
      try {
        (svgEl as any).pauseAnimations?.();
        if (currentTime != null) {
          try { (svgEl as any).setCurrentTime?.(currentTime); } catch {}
        }
      } catch {}
  ...
  (map as any).addImage(id, styleImage, { pixelRatio });
  styleImage.render();
