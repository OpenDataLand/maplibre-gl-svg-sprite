 ...
 */
export type SvgUrlParams = BaseSvgParams &
  SvgColorParams &
  SpriteTintParams &
  OverlayTextParams &
  OverlaySvgParams &
 ...
 * ```
 */
export function buildSvgUrl(params: SvgUrlParams): string {
  ...
  const queryString = buildQueryString(rest as Record<string, string | number | boolean | undefined>);
...
export class SvgUrlBuilder {
  private params: Partial<SvgUrlParams>;

  constructor(icon: string, protocol: string = 'svg') {
  ...
  size(width: number, height?: number): this {
    ...
    this.params.height = height ?? width;
    return this;
  }
   ...
   */
  colors(opts: { fg?: string; bg?: string; color?: string }): this {
    if (opts.fg) this.params.fg = opts.fg;
    if (opts.bg) this.params.bg = opts.bg;
    if (opts.color) this.params.color = opts.color;
    return this;
  }
   ...
   */
  build(): string {
    return buildSvgUrl(this.params as SvgUrlParams);
  }
}
 ...
 * ```
 */
export function svgUrl(icon: string, protocol?: string): SvgUrlBuilder {
