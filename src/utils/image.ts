...
export function hasCreateImageBitmap(): boolean {
  ...
  if (globalScope && typeof globalScope.createImageBitmap === 'function') return true;
  const winScope = typeof window !== 'undefined' ? (window as any) : undefined;
 ...
 */
export function computeTargetSize(svg: string, pixelRatio: number): { width: number; height: number } {
 ...
 * @returns Promise resolving to Blob
 */
export async function canvasLikeToBlob(
  ...
  return await canvasToBlob(canvas as HTMLCanvasElement, type, quality);
}
 ...
 */
export async function createImageBitmapFromCanvas(canvas: HTMLCanvasElement): Promise<ImageBitmap | HTMLCanvasElement> {
  if (hasCreateImageBitmap()) return await createImageBitmap(canvas);
 ...
 */
export async function svgToBitmap(svgString: string, pixelRatio: number): Promise<{ bitmap: ImageBitmap | HTMLCanvasElement; width: number; height: number }> {
  ...
  if (hasCreateImageBitmap()) {
    try {
      const bitmap = await createImageBitmap(blob, { resizeWidth: width, resizeHeight: height, resizeQuality: 'high' } as any);
      return { bitmap, width: (bitmap as any).width || width, height: (bitmap as any).height || height };
    } catch (e) {
