...
export type ProtocolRegistry = Record<string, { 1: SpriteAssets; 2?: SpriteAssets; [ratio: number]: SpriteAssets | undefined }>;
 ...
 */
function normalizeIcons(icons: SvgInput[] | Record<string, string>): SvgInput[] {
...
export async function generateBrowserSprite({ imgs, pixelRatio = 1 }: { imgs: SvgInput[] | Record<string, string>; pixelRatio?: number }): Promise<SpriteResult> {
  ...
  type PackedItem = { id: string; x: number; y: number; width: number; height: number; bitmap: ImageBitmap | HTMLCanvasElement };
  let x = 0, maxH = 0; const items: PackedItem[] = [];
  for (const it of prepared) { items.push({ id: it.id, x, y: 0, width: it.width, height: it.height, bitmap: it.bitmap }); x += it.width; if (it.height > maxH) maxH = it.height; }
  const width = Math.max(1, x); const height = Math.max(1, maxH);
  const canvas = document.createElement('canvas'); canvas.width = width; canvas.height = height;
  const ctx = canvas.getContext('2d'); if (!ctx) throw new Error('Could not get 2D context');
  for (const it of items) ctx.drawImage(it.bitmap as CanvasImageSource, it.x, it.y);
  const spriteBlob = await canvasToBlob(canvas, 'image/png');
  const spriteURL = URL.createObjectURL(spriteBlob);
  const json: SpriteJSON = {}; for (const it of items) json[it.id] = { width: it.width, height: it.height, x: it.x, y: it.y, pixelRatio };
...
export class SpriteBuilder {
  ...
  private _items: SvgInput[] = [];
  private _dirty = true;
  private _built: { json: SpriteJSON; canvas: HTMLCanvasElement; width: number; height: number } | null = null;
  private _urls: { sprite: string | null; json: string | null } = { sprite: null, json: null };
   ...
   */
  constructor({ pixelRatio = 1 }: { pixelRatio?: number } = {}) { this._pixelRatio = pixelRatio; }

  /** Set pixel ratio (must be positive number) */
  setPixelRatio(ratio: number) { if (!(typeof ratio === 'number' && isFinite(ratio) && ratio > 0)) throw new Error('pixelRatio must be a positive number'); this._pixelRatio = ratio; this._markDirty(); }

  /** Add an SVG icon to the sprite */
  addSvg(id: string, svg: string) {
    if (!id || typeof id !== 'string') throw new Error('addSvg requires a string id');
    if (typeof svg !== 'string') throw new Error('addSvg requires svg string content');
    if (this._items.some(it => it.id === id)) {
      try { console.warn(`[SpriteBuilder] duplicate id '${id}' added; last definition wins in JSON.`); } catch {}
    }
    this._items.push({ id, svg }); this._markDirty();
  }

  /** Add multiple SVG icons at once (array or record) */
  addSvgs(icons: SvgInput[] | Record<string, string>) { for (const { id, svg } of normalizeIcons(icons)) this.addSvg(id, svg); }

  /** Add an SVG icon only if not already present */
  addSvgIfAbsent(id: string, svg: string): boolean { if (this._items.some(it => it.id === id)) return false; this.addSvg(id, svg); return true; }

  /** Export sprite sheet as object URL */
  async exportSpriteURL(): Promise<string> { const built = await this._ensureBuilt(); if (this._urls.sprite) URL.revokeObjectURL(this._urls.sprite); const blob = await canvasToBlob(built.canvas, 'image/png'); this._urls.sprite = URL.createObjectURL(blob); return this._urls.sprite; }

  /** Export JSON metadata as object URL */
  async exportJSONURL(): Promise<string> { const built = await this._ensureBuilt(); if (this._urls.json) URL.revokeObjectURL(this._urls.json); const blob = new Blob([JSON.stringify(built.json)], { type: 'application/json' }); this._urls.json = URL.createObjectURL(blob); return this._urls.json; }

  /** Export both sprite and JSON URLs */
  async exportURLs(): Promise<{ sprite: string; json: string }> { const [sprite, json] = await Promise.all([ this.exportSpriteURL(), this.exportJSONURL() ]); return { sprite, json }; }

  /** Export URLs along with layout size */
  async exportBundle(): Promise<{ sprite: string; json: string; width: number; height: number }> {
  ...
  async exportJSON(): Promise<SpriteJSON> { const built = await this._ensureBuilt(); return built.json; }

  /** Export canvas element */
  async exportCanvas(): Promise<HTMLCanvasElement> { const built = await this._ensureBuilt(); return built.canvas; }

  /** Export sprite assets (JSON + PNG bytes) */
  async exportAssets(): Promise<SpriteAssets> { const built = await this._ensureBuilt(); const blob = await canvasToBlob(built.canvas, 'image/png'); const png = await blob.arrayBuffer(); return { json: built.json, png }; }

  /** Clean up resources and revoke URLs */
  destroy() { if (this._urls.sprite) URL.revokeObjectURL(this._urls.sprite); if (this._urls.json) URL.revokeObjectURL(this._urls.json); this._urls = { sprite: null, json: null }; this._built = null; }

  /** Revoke a specific object URL */
  expire(url?: string | null) { if (!url) return; try { URL.revokeObjectURL(url); } catch {} if (this._urls.sprite === url) this._urls.sprite = null; if (this._urls.json === url) this._urls.json = null; }
  private _markDirty() { this._dirty = true; this._built = null; if (this._urls.sprite) { URL.revokeObjectURL(this._urls.sprite); this._urls.sprite = null; } if (this._urls.json) { URL.revokeObjectURL(this._urls.json); this._urls.json = null; } }
  private async _ensureBuilt() { if (!this._dirty && this._built) return this._built; const imgs = this._items.map(({ id, svg }) => ({ id, svg })); const { spriteURL, jsonURL, json, canvas, width, height } = await generateBrowserSprite({ imgs, pixelRatio: this._pixelRatio }); if (spriteURL) URL.revokeObjectURL(spriteURL); if (jsonURL) URL.revokeObjectURL(jsonURL); this._built = { json, canvas, width, height }; this._dirty = false; return this._built; }
   ...
   */
  static registerMapLibreProtocol(
    maplibre: MapLibreLike,
    protocol: string,
    registry: Record<string, { 1: SpriteAssets; 2?: SpriteAssets }>,
    ...
    const served: Record<string, { [ratio: number]: { png: boolean; json: boolean } }> = {};
    const timers: Record<string, { [ratio: number]: any }> = {};
    const scheduleTTL = (k: string, r: number) => {
      if (!options.ttlMs) return;
      timers[k] = timers[k] || {};
      if (timers[k][r]) return;
      timers[k][r] = setTimeout(() => {
        try {
          if (registry[k]) delete (registry[k] as any)[r];
          if (timers[k]) delete timers[k][r];
          const hasAny = registry[k] && Object.keys(registry[k]).some(x => /^\d+$/.test(x));
          if (!hasAny) delete registry[k];
        } catch {}
      }, options.ttlMs);
    };
    const handler = (
      requestParams: { url: string },
      callback?: (err: any, response?: { data: ArrayBuffer | string | SpriteJSON }) => void
    ) => {
      const respond = (res: { data: ArrayBuffer | string | SpriteJSON }) => {
        if (typeof callback === 'function') { callback(null, res); return undefined; }
        return Promise.resolve(res);
      };
      const fail = (err: any) => {
        if (typeof callback === 'function') { callback(err); return undefined; }
        return Promise.reject(err);
      };
      try {
      const url = requestParams.url;
      if (typeof window !== 'undefined' && (window as any).__DEBUG_SVG_PROTOCOL__) {
        console.debug('[registerSVGProtocol] request', url);
      }
      const prefix = protocol + '://';
        if (!url.startsWith(prefix)) return fail(new Error('Unhandled protocol URL')) as any;
        const rest = url.slice(prefix.length);
        const path = rest.split('?')[0];
        const isJSON = /\.json$/i.test(path);
        const isPNG = /\.png$/i.test(path);
        if (!isJSON && !isPNG) return fail(new Error('Unsupported sprite request: ' + url)) as any;
        ...
        if (!entry) {
          ...
          return fail(new Error('Sprite key not found: ' + decodedKey + hint)) as any;
        }
        const assets = (ratio === 2 && entry[2]) ? entry[2]! : entry[1];
        if (!assets) return fail(new Error('Sprite assets missing for ratio ' + ratio)) as any;
        ...
        if (options.oneShot) {
          ...
          if (isJSON) served[decodedKey][ratio].json = true; if (isPNG) served[decodedKey][ratio].png = true;
          const flags = served[decodedKey][ratio];
          if (flags.png && flags.json) {
            delete (registry[decodedKey] as any)[ratio]; delete served[decodedKey][ratio];
            const hasAny = Object.keys(registry[decodedKey] || {}).some(k => /^\d+$/.test(k)); if (!hasAny) delete registry[decodedKey];
            if (timers[decodedKey] && timers[decodedKey][ratio]) { try { clearTimeout(timers[decodedKey][ratio]); } catch {} delete timers[decodedKey][ratio]; }
          }
        }
        return respond(res) as any;
      } catch (e) {
 ...
 */
export async function buildSpriteRegistryFromIcons(
  key: string,
  icons: SvgInput[] | Record<string, string>,
  ratios: number[] = [1, 2]
): Promise<ProtocolRegistry> {
  const byRatio: { [ratio: number]: SpriteAssets } = {} as any;
  const list = normalizeIcons(icons);
  for (const r of ratios) { const sb = new SpriteBuilder({ pixelRatio: r }); list.forEach(({ id, svg }) => sb.addSvg(id, svg)); byRatio[r] = await sb.exportAssets(); }
  const entry: any = {}; for (const r of ratios) entry[r] = byRatio[r];
  return { [key]: entry } as ProtocolRegistry;
}
 ...
 */
export async function registerProtocolFromIcons(
  maplibre: MapLibreLike,
  protocol: string,
  key: string,
  icons: SvgInput[] | Record<string, string>,
 ...
 * @returns Promise resolving to unregister function
 */
export async function registerOneShotSpriteFromIcons(
  maplibre: MapLibreLike,
  protocol: string,
  key: string,
  icons: SvgInput[] | Record<string, string>,
...
export function registerSVGProtocol(
  ...
  const handler = async (
    requestParams: { url: string },
    callback?: (err: any, response?: { data: ArrayBuffer }) => void
  ) => {
    const respond = (value: { data: ArrayBuffer }) => {
    ...
    try {
      const url = requestParams.url;
      const prefix = protocol + '://';
      if (!url.startsWith(prefix)) return fail(new Error('Unhandled protocol URL'));
      const rest = url.slice(prefix.length);
      ...
      if (!original) return fail(new Error('SVG not found: ' + id));
      ...
      if (typeof document === 'undefined') {
        if (typeof OffscreenCanvas === 'undefined') throw new Error('OffscreenCanvas is required in Worker context');
